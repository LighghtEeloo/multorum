...
- Forward a candidate group only after updating the canonical workspace and the current rulebook, and only when every live worker in that group is non-`ACTIVE`.
...
4. Create one worker per perspective by default. Create multiple workers from the same perspective only when you intentionally want a candidate group.
