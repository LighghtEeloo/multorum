...
use crate::runtime::{
    CreateWorker, FsOrchestratorService, OrchestratorService, WorkerId,
};

use super::{
    args_or_empty, dispatch_tool, extract_payload, extract_reply, list_resources_result,
    list_tools_result, optional_bool, optional_str, optional_string_list, required_str,
    resource_success, server_info,
};
...
impl OrchestratorHandler {
    ...
    pub fn new(service: FsOrchestratorService) -> Self {
        ...
        let resources =
            list_resources_result(&crate::mcp::resource::orchestrator::descriptors());
    ...
    pub fn dispatch(
        ...
        match name {
            ...
            | "resolve_worker" => {
                ...
                dispatch_tool(
                    self.service
                        .resolve_worker(worker_id, extract_reply(&args), extract_payload(&args)),
                )
            }
            | "revise_worker" => {
                let worker_id = parse_worker_id(required_str(&args, "worker_id")?)?;
                dispatch_tool(
                    self.service
                        .revise_worker(worker_id, extract_reply(&args), extract_payload(&args)),
                )
            }
            | "discard_worker" => {
                let worker_id = parse_worker_id(required_str(&args, "worker_id")?)?;
    ...
    pub fn read(&self, uri: &str) -> Result<ReadResourceResult, ErrorData> {
        match uri {
            ...
            | "multorum://orchestrator/perspectives" => {
                let perspectives =
                    self.service.list_perspectives().map_err(runtime_to_error)?;
                resource_success(uri, &perspectives)
            }
            | "multorum://orchestrator/workers" => {
                let workers = self.service.list_workers().map_err(runtime_to_error)?;
    ...
    fn read_worker_resource(&self, uri: &str) -> Result<ReadResourceResult, ErrorData> {
        let path = uri
            .strip_prefix("multorum://orchestrator/workers/")
            .unwrap_or("");
        ...
        let worker_id = worker_id_str.parse::<WorkerId>().map_err(|e| {
            ErrorData::invalid_params(format!("invalid worker id: {e}"), None)
        })?;

        match sub {
            | None => {
                let detail =
                    self.service.get_worker(worker_id).map_err(runtime_to_error)?;
                resource_success(uri, &detail)
            }
            | Some("contract" | "transcript" | "checks") => Err(ErrorData::new(
                rmcp::model::ErrorCode::RESOURCE_NOT_FOUND,
                format!("resource not yet implemented: {uri}"),
                None,
            )),
            | Some(other) => Err(ErrorData::new(
                rmcp::model::ErrorCode::RESOURCE_NOT_FOUND,
...
fn parse_perspective(
    s: &str,
) -> Result<crate::perspective::PerspectiveName, ErrorData> {
    s.parse()
        .map_err(|e| ErrorData::invalid_params(format!("invalid perspective name: {e}"), None))
}
