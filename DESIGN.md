...
forward_request = "expand-boundary" # optional, for blocker reports that want auto-forward
```
...
`forward_request` is typed metadata on blocker reports. It exists so the orchestrator can distinguish "this blocker needs perspective evolution and replay" from ordinary blocking questions without inferring intent from free-form prose. Auto-forward on `resolve` is allowed only when the latest blocking report carries one of these typed requests and the whole bidding group still satisfies the normal forward rules.
...
- `multorum worker resolve <worker> [--reply-to <sequence>] (--body-text <text> | --body-path <file>) [--artifact <file>]...` — Publish a `resolve` bundle to a blocked worker inbox. `--reply-to` correlates the resolve with an earlier outbox sequence number. The required body carries resolution context for the worker. The worker returns to `ACTIVE` when it acknowledges that inbox message.
